...
# это набор пакетов с их версиями, определенный двумя файлами. Project.toml - список пакетов. Manifest.toml - полный список всех зависимостей с точными версиями

# Что такое пакет (package), как добавить новый пакет?
#добавляется командой add в Pkg-режиме или Pkg.add()

# Как начать разрабатывать чужой пакет?
#добавить пакет, клонировать репозиторий к себе, перейти в каталог пакета
...
c = DataFrame(x=[5,6], y=[7,8])
CSV.write("data.csv", c)
...
а) из файла .jl julia --project=/путь/script.jl
б) из текста команды? (см. флаг -e) julia --project=. -e 'myfunction()'
=#
